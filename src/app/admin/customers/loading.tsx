...
export default function AdminCustomersLoading() {
  return (
    <div className="space-y-6 animate-pulse font-sans" aria-busy="true" aria-label="Đang tải danh sách khách hàng...">
      {/* Title skeleton */}
        ...
        <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
          ...
          <div className="flex gap-2">
            {Array.from({ length: 4 }).map((_, i) => (
              <div key={i} className="h-8 w-24 bg-[#F0EDE8] rounded-[2px]" />
            ))}
          </div>
        </div>
          ...
          {Array.from({ length: 10 }).map((_, i) => (
            <div key={i} className="py-4 px-6 block md:flex items-center">
              <div className="flex-1 grid grid-cols-1 md:grid-cols-6 gap-4 items-center">
                ...
                <div className="h-6 w-20 bg-[#F0EDE8] rounded-[2px]" />
                <div className="h-4 w-20 bg-[#F0EDE8] rounded-[2px] font-mono" />
                <div className="h-4 w-12 bg-[#F0EDE8] rounded-[2px] font-mono" />
                <div className="h-4 w-20 bg-[#F0EDE8] rounded-[2px] font-mono" />
